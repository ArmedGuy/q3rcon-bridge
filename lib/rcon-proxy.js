...
var needsOptions = ["bindHost", "servers", "authLib"];
function rconproxy(options) {
	...
	this.nat = udpnat.createServer(options, function(msg, sender, callback) {
		if(arr_compare(rconheader, msg.slice(0, 8))) {
			var str = msg.toString('utf8').substr(4).trim().replace(trim,"");
			var parts = str.split(" ");
			rawcmd = parts.shift();
			...
			var userServer = password.indexOf("/") > -1 ? password.split("/") : ["",""];
			if(!(userServer[1] in $proxy.options.servers)) {
				$proxy.emit("badserver", {ip: sender.address, port: sender.port}, userServer[1], command);
				return callback(false);
			}
			...
			$proxy.authlib.auth(authuser[0], authuser[1], userServer[1], function(loggedIn) {
				if(loggedIn) {
					...
					rcon_msg = [rawcmd, $proxy.options.servers[userServer[1]].password, command].join(" ");

					var buf = new Buffer(4 + rcon_msg.length);
					msg.copy(buf, 0, 0, 4);
					buf.write(rcon_msg, 4, rcon_msg.length);
					return callback(buf, $proxy.options.servers[userServer[1]]);
				} else {
