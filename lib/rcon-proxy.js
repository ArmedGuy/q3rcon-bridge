	...
	this.nat = udpnat.createServer(options, function(msg, sender, callback) {
		if(arr_compare(msg.slice(0, 8).toJSON(), rconheader)) {
			...
			$proxy.authlib.auth(authuser[0], authuser[1], function(loggedIn) {
				if(loggedIn) {
					$proxy.emit("command", {user: authuser[0], ip: sender.address, port: sender.port}, command);
					...
					$proxy.emit("badauth", {user: authuser[0], ip: sender.address, port: sender.port}, command);
					return callback(msg);
				}
			});
		} else {
